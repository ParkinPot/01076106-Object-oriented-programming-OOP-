...
class Account:
    ...
    def withdraw(self, channel_id, amount):
        ...
        self.add_transaction(Transaction("W", channel_id, amount, self.__balance))
    ...
    def deposit(self, channel_id, amount):
        ...
        self.__balance += amount
        self.add_transaction(Transaction("D", channel_id, amount, self.__balance))
...
class SavingAccount(Account):
    ...
    def withdraw(self, channel_id, amount):
        ...
        self.balance -= amount
        self.add_transaction(Transaction("W", channel_id, amount, self.balance))
        
        return "Success"
    
    def calculate_interest(self, time):
        if type(time) is int:
            interest = self.balance * 0.005 * time
            self.balance += interest
            self.add_transaction(Transaction("I", None, interest, self.balance))
            return interest
...
class FixedAccount(Account):
    def __init__(self, account_no, user, time, balance = 0.0):
        super().__init__(account_no, user, balance)
        self.__time_period = time
    ...
    def calculate_interest(self, time):
        interest = self.balance * 0.025  * time/self.time
        self.balance += interest
        return interest
    ...
    def deposit(self, channel_id, amount):
        ...
        self.balance += amount
        self.add_transaction(Transaction("D", channel_id, amount, self.balance))
    ...
    def withdraw(self, channel_id, amount):
        ...
        deposit_date = getattr(self, "_FixedAccount__deposit_date", current_date)  
        maturity_date = deposit_date + timedelta(days=365)  

        if current_date >= maturity_date:
            full_interest = self.calculate_interest(self.time)
            self.add_transaction(Transaction("I", None, full_interest, self.balance))
        else:
            reduced_time = (current_date - deposit_date).days / 30  
            reduced_interest = self.calculate_interest(reduced_time)
            self.add_transaction(Transaction("I", None, reduced_interest, self.balance))
...
class ATMMachine(TransactionChannel):
    ...
    def deposit(self, account, amount):
        if account.deposit(self.channel_id, amount) == "Success":
            self.__money += amount
            return "Success"
        else:
            return "Error : amount must be greater than 0"
    ...
    def withdraw(self, account, amount):
        if account.withdraw(self.channel_id, amount) == "Success":
            self.__money -= amount
            return "Success"
        else:
            return "Error"
...
class EDCMachine(TransactionChannel):
    ...
    def calculate_cashback(self, debit_card, amount):
        if not isinstance(debit_card, ShoppingDebitCard):
            return "Error"
    ...
    def pay(self, debit_card, amount):
        ...
        customer_account.withdraw(self.channel_id, amount)

        self.__merchant_account.deposit(self.channel_id, amount)

...
class Subject:
    ...
    teacher = property(get_teacher,assign_teacher)
    pass
...
def enroll_to_subject(student, subject):
    ...
    student_enrollment = Enrollment(student, subject)
    enrollment_list.append(student_enrollment)
...
def drop_from_subject(student, subject):
    ...
    for enroll in enrollment_list:
        if enroll.student.student_id == student.student_id and enroll.subject.subject_id == subject.subject_id:
...
def search_enrollment_subject_student(subject, student):
    if not isinstance(subject, Subject) and not isinstance(student, Student):
        return "Error"
    
    for enroll in enrollment_list:
        if enroll.subject == subject and enroll.student == student:
            return enroll
...
def search_student_enroll_in_subject(subject):
    ...
    for enroll in enrollment_list:
        if enroll.subject == subject:
            student_enrollment_list.append(enroll)
    return(student_enrollment_list)
...
def search_subject_that_student_enrolled(student):
    ...
    for enroll in enrollment_list:
        if enroll.student == student:
            subject_enrollment_list.append(enroll)

    if len(subject_enrollment_list) > 0:
        return subject_enrollment_list
    else:
        return "Not Found"
...
def get_no_of_student_enrolled(subject):
    enrolled_students = search_student_enroll_in_subject(subject)
...
def get_student_record(student):
    ...
    if not enrollments:
        return record
    for enroll in search_subject_that_student_enrolled(student):
        grade_list = []
        subject_id = (enroll.subject.subject_id)
        subject_name = (enroll.subject.subject_name)
        grade_list.append(subject_name)
        grade_list.append(enroll.grade)
        record[subject_id] = grade_list
    return record
...
def get_student_GPS(student):
    ...
    for enrollment in search_subject_that_student_enrolled(student):
        total_grade += grade_to_count(enrollment.grade) * (enrollment.subject.credit)
        total_credit += grade_to_count(enrollment.grade)
